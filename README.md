...
Download the JMX Prometheus Exporter jar (needs internet access on the VM):
```bash
   ...
   ```
Copy `deploy/jmx-exporter.yml` to `/home/apache_kafka/kafka_2.13-4.3.1/config/jmx-exporter.yml`.
Start the broker with the javaagent attached:
```bash
   ...
   ```
Verify: `curl http://localhost:7071/metrics` should show `kafka_server_*` lines.
Repeat similarly for the controller process if it runs separately (use a different port, e.g. 7072).
...
Next real steps
Per-connector status (`GET /connectors/{name}/status`) and per-task
JMX metrics — needs a real connector running to verify shapes against
(deferred, see above).
Swap the HTTP/JSON Hub client for real gRPC streaming.
Add mTLS, local guardrail enforcement, credential custody.
Package as Helm chart / Docker image / RPM/DEB.
